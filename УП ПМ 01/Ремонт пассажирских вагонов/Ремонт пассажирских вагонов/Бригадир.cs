...
{
    public partial class Бригадир : Form
    {
        int s;
        public Бригадир()
        {
        ...
        private void Бригадир_Load(object sender, EventArgs e) // Метод вызванный самой программой при добавлении в табличную часть объектов, представленные ниже
        {
            ...
            for (int i = 0; i < Таблица.RowCount; i++)
            {
                ...
                for (int j = 0; j < Таблица.ColumnCount; j++)
                    if (Таблица.Rows[i].Cells[j].Value != null)
                        if (Таблица.Rows[i].Cells[j].Value.ToString().Contains(Таблица.Text))
                        {
        ...
        Point Mouse;
        private void Бригадир_MouseMove(object sender, MouseEventArgs e) // Навигация формы курсором с помощью левой кнопкой мыши
        {
        ...
        private void Бригадир_MouseDown(object sender, MouseEventArgs e)
        {
